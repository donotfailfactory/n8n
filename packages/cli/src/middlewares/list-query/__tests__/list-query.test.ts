...
describe('List query middleware', () => {
	...
	describe('Query filter', () => {
		...
		test('should parse valid filter', async () => {
			mockReq.query = { filter: '{ "name": "나의 비즈니스 루틴" }' };

			await filterListQueryMiddleware(...args);

			expect(mockReq.listQueryOptions).toEqual({ filter: { name: '나의 비즈니스 루틴' } });
			expect(nextFn).toBeCalledTimes(1);
		});

		test('should ignore invalid filter', async () => {
			mockReq.query = { filter: '{ "name": "나의 비즈니스 루틴", "foo": "bar" }' };
	...
	describe('Combinations', () => {
		test('should combine filter with select', async () => {
			mockReq.query = { filter: '{ "name": "나의 비즈니스 루틴" }', select: '["name", "id"]' };
		...
		test('should combine filter with pagination options', async () => {
			mockReq.query = { filter: '{ "name": "나의 비즈니스 루틴" }', skip: '1', take: '2' };
